...
class CheckDBStructure
{
    ...
    private function checkDB(\stdClass $tableStructureFlat)
    {
        ...
        foreach ($tableStructureFlat as $table => $columns) {
            ...
            if (!$result->fetchColumn()) {
                $this->output->writeln('    The table `' . $table . '` is not exists.');
                ++$errors;
            } else {
                foreach ($columns as $column => $value) {
                    ...
                    if (!$result) {
                        $this->output->writeln('    The column `' . $table . '`.`' . $column . '` is not exists.');
                        ++$errors;
                    }
                }// endfoerach;
                unset($column, $value);
        ...
        if ($errors === 0) {
            $this->output->writeln('    <success>[✓] All tables and columns are checked and exists.</success>');
        } else {
            $this->output->writeln('    <warning>[!] There is an error found, please read details.</warning>');
        }
    }// checkDB
     ...
     * @param \stdClass $tableStructureFlat
     */
    private function displayResult(\stdClass $tableStructure, \stdClass $tableStructureFlat)
    {
        $this->output->writeln('  Table structure based on JSON: =============');
        print_r($tableStructure);
        $this->output->writeln('  End table structure based on JSON: =========');

        $this->output->writeln('  Flatten: ===================================');
        print_r($tableStructureFlat);
        $this->output->writeln('  End flatten: ===============================');

        $this->output->writeln('  Checking structure with the database: ======');
        $this->checkDB($tableStructureFlat);
        $this->output->writeln('  End check structurewith the database: ======');
    ...
    public function run(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->displayResult($tableStructure, $tableStructureFlat);

        unset($tableStructure, $tableStructureClone, $tableStructureFlat);
        unset($jsonFile, $jsonFolder);
    }// run
